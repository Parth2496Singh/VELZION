...
const Dashboard = () => {
  ...
  const [message, setMessage] = useState(''); // Used for destroy notifications
  ...
  const handleSaveIntegration = (e) => {
    ...
    axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/projects/`, {
      ...
      setIamRepoUrl('');
      setIamRoleArn('');
      setTimeout(() => setIamStatus(''), 5000); // Clear success message after 5s
    }).catch(err => {
  ...
  // --- FINOPS ENGINE (CRITICAL - PRESERVED) ---
  ...
  return (
    <div className="vz-dashboard">
      ...
      <style>{`
        ...
        /* SIDEBAR */
        .vz-sidebar { width: 260px; background-color: var(--bg-card); border-right: 1px solid var(--border-color); padding: 32px 24px; display: flex; flex-direction: column; }
        ...
        /* MAIN LAYOUT */
        .vz-main { flex: 1; padding: 40px 60px; overflow-y: auto; }
        ...
        .vz-btn-calc { background-color: var(--accent-emerald-dim); color: var(--accent-emerald); border: 1px solid rgba(16, 185, 129, 0.2); }
        .vz-btn-calc:hover { background-color: rgba(16, 185, 129, 0.15); }
        ...
        /* FORMS & INPUTS */
        .vz-section { background-color: var(--bg-card); padding: 32px; border-radius: 16px; margin-bottom: 32px; border: 1px solid var(--border-color); box-shadow: 0 4px 24px rgba(0,0,0,0.2); }
        .vz-input-group label { display: block; margin-bottom: 10px; color: #d1d5db; font-size: 13px; font-weight: 500; }
        .vz-input { width: 100%; padding: 14px 16px; border-radius: 8px; border: 1px solid var(--border-color); background-color: var(--bg-dark); color: #fff; box-sizing: border-box; font-family: 'JetBrains Mono', monospace; font-size: 13px; transition: 0.2s; }
        .vz-input:focus { outline: none; border-color: var(--accent-cyan); box-shadow: 0 0 0 3px var(--accent-cyan-dim); }
        
        /* ALERT NOTIFICATIONS */
        .vz-message { margin-top: 24px; padding: 16px; background: var(--accent-cyan-dim); border: 1px solid rgba(0, 229, 255, 0.2); border-radius: 8px; color: var(--accent-cyan); font-weight: 500; font-size: 14px; display: flex; align-items: center; gap: 10px; }
        
        /* ENVIRONMENTS LIST */
        .vz-listener-banner { display: flex; align-items: center; justify-content: space-between; background: linear-gradient(90deg, var(--accent-cyan-dim), transparent); border-left: 3px solid var(--accent-cyan); padding: 16px 24px; border-radius: 0 8px 8px 0; margin-bottom: 32px; }
        ...
        .vz-env-card { background-color: var(--bg-dark); padding: 28px; border-radius: 12px; border: 1px solid var(--border-color); position: relative; overflow: hidden; transition: 0.3s; }
        .vz-env-card:hover { border-color: #374151; box-shadow: 0 8px 30px rgba(0,0,0,0.3); }
        
        /* STATUS INDICATORS */
        .status-indicator-bar { position: absolute; left: 0; top: 0; bottom: 0; width: 4px; }
        .status-RUNNING .status-indicator-bar { background-color: var(--accent-emerald); box-shadow: 0 0 15px var(--accent-emerald); }
        ...
        .vz-badge { padding: 4px 10px; border-radius: 6px; font-size: 11px; font-weight: 700; letter-spacing: 0.5px; }
        .bg-green { background: var(--accent-emerald-dim); color: var(--accent-emerald); border: 1px solid rgba(16, 185, 129, 0.2); }
        .bg-gray { background: #1f232b; color: var(--text-muted); border: 1px solid var(--border-color); }
        ...
        /* FINOPS WIDGET */
        .vz-finops { background-color: var(--bg-card); border-radius: 10px; padding: 20px; border: 1px solid var(--border-color); }
        .vz-finops-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; }
        .vz-finops-title { margin: 0; font-size: 11px; color: var(--text-muted); text-transform: uppercase; font-weight: 700; letter-spacing: 1px; }
        ...
        .vz-instance { margin: 0; color: #6b7280; font-family: 'JetBrains Mono', monospace; font-size: 12px; background: #1a1d24; padding: 6px 10px; border-radius: 6px; }
        
        .vz-btn-terminate { background-color: transparent; color: var(--accent-red); border: 1px solid var(--accent-red); padding: 10px 20px; border-radius: 8px; cursor: pointer; font-size: 13px; font-weight: 600; transition: 0.2s; }
        .vz-btn-terminate:hover:not(:disabled) { background-color: var(--accent-red); color: #fff; box-shadow: 0 0 15px var(--accent-red-dim); }
      ...
      <div className="vz-main">
        ...
        {activeTab === 'environments' && (
          <>
            ...
            {/* NEW: Webhook Listener Hero Banner */}
            ...
            <div style={{ background: 'transparent', border: 'none', padding: 0 }}>
              {!(environments || []).length ? (
                <div style={{ textAlign: 'center', padding: '80px 20px', border: '1px dashed var(--border-color)', borderRadius: '16px', backgroundColor: 'var(--bg-card)' }}>
                        ...
                        <div className="vz-env-meta">
                          {env.status === 'DESTROYED' ? (
                            ...
                            <span style={{ display: 'flex', alignItems: 'center', gap: '8px'}}>
                              Deployment Live: <a href={env.dns_prefix} target="_blank" rel="noreferrer" className="vz-link">{env.dns_prefix} ↗</a>
                            </span>
                          ) : (
        ...
        {activeTab === 'iam' && (
          <div className="vz-section">
            ...
            <div style={{ backgroundColor: 'var(--bg-dark)', padding: '24px', borderRadius: '12px', border: '1px solid var(--border-color)', marginBottom: '32px' }}>
            ...
            <form onSubmit={handleSaveIntegration} style={{ display: 'grid', gap: '24px', backgroundColor: 'var(--bg-dark)', padding: '24px', borderRadius: '12px', border: '1px solid var(--border-color)' }}>
