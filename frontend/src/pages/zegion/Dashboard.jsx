...
import { 
  ...
  Trash2, ExternalLink, Play, PauseCircle, Loader2, CloudCog, Activity, TerminalSquare
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      {/* ---------------- ZEGION COMMAND BAR ---------------- */}
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={springPage} className="glass-panel" style={{ padding: '1rem 1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'relative', zIndex: 10 }}>
        <div style={{ display: 'flex', gap: '0.5rem', position: 'relative' }}>
          
          {['environments', 'iam'].map(tab => (
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
            <select value={currentWorkspace || ''} onChange={(e) => { if (e.target.value) navigate(`/zegion/dashboard/${e.target.value}`); }} style={{ 
                background: 'var(--bg-layer-2)', border: 'var(--border-subtle)', color: 'var(--text-pure)', 
          ...
          <Link to="/zegion/finops" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', borderRadius: 'var(--radius-sm)', background: 'var(--bg-layer-2)', border: 'var(--border-subtle)', color: 'var(--text-pure)', fontSize: '0.9rem', fontWeight: 600, transition: 'all var(--transition-fast)' }}>
            <BarChart3 size={16} strokeWidth={1.5} style={{ color: 'var(--zg-purple-core)' }}/> ROI
          </Link>
        </div>
      </motion.div>
      ...
      <AnimatePresence mode="wait">
        {!currentWorkspace && (
          <motion.div key="blank-slate" initial={{ opacity: 0, filter: 'blur(10px)' }} animate={{ opacity: 1, filter: 'blur(0px)' }} exit={{ opacity: 0 }} transition={springPage} className="glass-panel" style={{ padding: '6rem 2rem', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
            <HolographicNode />
            <h2 style={{ fontSize: 'clamp(1.75rem, 4vw, 2.5rem)', color: 'var(--text-pure)', margin: '0 0 1rem 0', fontWeight: 800, letterSpacing: '-0.02em' }}>Awaiting Coordinates</h2>
            <p style={{ color: 'var(--text-muted)', fontSize: 'clamp(0.8125rem, 1.2vw, 1rem)', maxWidth: '500px', lineHeight: 1.6, margin: '0 auto' }}>
      ...
      <AnimatePresence mode="wait">
        {currentWorkspace && activeTab === 'environments' && (
          <motion.div key="environments" variants={containerVariants} initial="hidden" animate="show" exit="hidden" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            {/* FLEET TELEMETRY HUD with Sparklines */}
            {(environments || []).length > 0 && (
              <motion.div variants={itemVariants} className="glass-panel" style={{ padding: '1.5rem 2rem', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem', position: 'relative', overflow: 'hidden' }}>
                <svg style={{ position: 'absolute', bottom: 0, left: 0, width: '100%', height: '50px', opacity: 0.3, pointerEvents: 'none' }} preserveAspectRatio="none" viewBox="0 0 100 100">
                  <defs>
                    <linearGradient id="sparklineZg" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="var(--zg-purple-core)" stopOpacity="0.4" />
                      <stop offset="100%" stopColor="var(--zg-purple-core)" stopOpacity="0" />
                    </linearGradient>
                  </defs>
                  <path d={`M0,100 L0,${100 - metrics.cpu[0]} ${metrics.cpu.map((c, i) => `L${i * 5.2},${100 - c}`).join(' ')} L100,100 Z`} fill="url(#sparklineZg)" />
                  <polyline points={metrics.cpu.map((c, i) => `${i * 5.2},${100 - c}`).join(' ')} fill="none" stroke="var(--zg-purple-core)" strokeWidth="1" />
                </svg>
                <div style={{ borderRight: 'var(--border-subtle)', position: 'relative', zIndex: 1 }}>
                  ...
                  <p style={{ margin: 0, fontSize: '2rem', fontWeight: 800, color: 'var(--text-pure)', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <Activity size={20} style={{ color: 'var(--zg-purple-core)' }} strokeWidth={1.5} /> {(environments || []).filter(e => e.status === 'RUNNING').length}
                  </p>
                </div>
                <div style={{ borderRight: 'var(--border-subtle)', position: 'relative', zIndex: 1 }}>
                  <p style={{ margin: '0 0 0.5rem 0', fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase', fontWeight: 700 }}>Sleeping PRs</p>
                  <p style={{ margin: 0, fontSize: '2rem', fontWeight: 800, color: 'var(--text-pure)', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <PauseCircle size={20} style={{ color: 'var(--text-muted)' }} strokeWidth={1.5} /> {(environments || []).filter(e => e.status === 'SLEEPING').length}
                  </p>
                </div>
                <div style={{ position: 'relative', zIndex: 1 }}>
                  ...
                  <p style={{ margin: 0, fontSize: '1.25rem', fontWeight: 700, color: '#10b981', display: 'flex', alignItems: 'center', gap: '0.75rem', marginTop: '0.5rem' }}>
                    <span className="status-pulse" style={{ width: '10px', height: '10px', background: '#10b981', borderRadius: '50%', boxShadow: '0 0 10px rgba(16, 185, 129, 0.5)' }} /> Optimal
                  </p>
                </div>
              </motion.div>
            )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              {!(environments || []).length ? (
                <motion.div variants={itemVariants} className="glass-panel" style={{ textAlign: 'center', padding: '5rem 2rem' }}>
                  <Server size={48} strokeWidth={1.5} style={{ color: 'var(--text-muted)', margin: '0 auto 1.5rem auto', opacity: 0.5 }} />
                  <h3 style={{ color: 'var(--text-pure)', margin: '0 0 1rem 0', fontSize: 'clamp(1.5rem, 3vw, 2rem)', fontWeight: 800 }}>Infrastructure Offline</h3>
                  <p style={{ color: 'var(--text-muted)', fontSize: 'clamp(0.8125rem, 1.2vw, 0.95rem)', margin: '0 auto', maxWidth: '450px', lineHeight: 1.6 }}>
                ...
                (environments || []).map((env) => {
                  ...
                  if (env.status === 'RUNNING') { statusColor = "#10b981"; bgGlow = "rgba(16, 185, 129, 0.03)"; StatusIcon = Play; }
                  else if (env.status === 'SLEEPING') { statusColor = "var(--zg-purple-core)"; bgGlow = "var(--zg-purple-glow)"; StatusIcon = PauseCircle; }
                  else if (env.status === 'PROVISIONING' || env.status === 'DESTROYING...') { statusColor = "#f59e0b"; bgGlow = "rgba(245, 158, 11, 0.03)"; StatusIcon = Loader2; }
                  else if (env.status === 'DESTROYED') { statusColor = "#ef4444"; StatusIcon = Trash2; }

                  return (
                    <motion.div key={env.id} variants={itemVariants} layout className="glass-panel" style={{ background: `linear-gradient(145deg, var(--bg-layer-1), ${bgGlow})`, borderLeft: `4px solid ${statusColor}`, position: 'relative', overflow: 'hidden' }}>
                      <div style={{ padding: '2rem' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem' }}>
                          <div>
                            <h3 style={{ display: 'flex', alignItems: 'center', gap: '1rem', margin: '0 0 0.75rem 0', fontSize: 'clamp(1.5rem, 3vw, 1.75rem)', fontWeight: 800 }}>
                              <span style={{ color: 'var(--text-muted)', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', background: 'var(--bg-layer-2)', padding: '4px 8px', borderRadius: 'var(--radius-sm)' }}>Pull Request</span> 
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', fontSize: '0.9rem' }}>
                               ...
                               env.dns_prefix ? (
                                <span style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', color: 'var(--text-pure)' }}>
                                  External Endpoint: 
                                  <a href={env.dns_prefix} target="_blank" rel="noreferrer" style={{ color: 'var(--text-pure)', background: 'var(--bg-layer-2)', border: 'var(--border-subtle)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '4px 12px', borderRadius: 'var(--radius-sm)', fontWeight: 600, fontSize: '0.85rem' }}>
                                    {env.dns_prefix} <ExternalLink size={12} strokeWidth={1.5} />
                                  </a>
                                </span>
                          ...
                          <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', borderRadius: 'var(--radius-sm)', fontWeight: 700, fontSize: '0.8rem', textTransform: 'uppercase', color: statusColor, background: 'var(--bg-layer-2)', border: `1px solid ${statusColor}` }}>
                            <StatusIcon size={14} strokeWidth={2} style={{ animation: env.status.includes('ING') ? 'spin 2s linear infinite' : 'none' }} />
                            {env.status}
                          </span>
                        </div>
                        ...
                        <AnimatePresence>
                          {stats && (
                            <motion.div layout initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} style={{ overflow: 'hidden' }}>
                              <div style={{ background: stats.isFrozen ? 'var(--zg-purple-glow)' : 'var(--bg-layer-2)', border: stats.isFrozen ? '1px dashed var(--zg-purple-border)' : 'var(--border-subtle)', borderRadius: 'var(--radius-sm)', padding: '1.5rem', marginTop: '1.5rem' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', paddingBottom: '1rem', borderBottom: 'var(--border-subtle)' }}>
                                  <p style={{ margin: 0, fontSize: '0.8rem', color: 'var(--text-muted)', textTransform: 'uppercase', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                    <BarChart3 size={14} strokeWidth={1.5} style={{ color: '#10b981' }} />
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1.5rem' }}>
                                  <div><p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: '0.5rem', textTransform: 'uppercase', fontWeight: 600 }}>Uptime</p><p style={{ margin: 0, fontSize: 'clamp(1rem, 2vw, 1.5rem)', fontWeight: 700, fontFamily: 'var(--font-mono)', color: stats.isFrozen ? 'var(--text-muted)' : 'var(--text-pure)' }}>{stats.uptimeString}</p></div>
                                  <div><p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: '0.5rem', textTransform: 'uppercase', fontWeight: 600 }}>Spot Rate</p><p style={{ margin: 0, fontSize: 'clamp(1rem, 2vw, 1.5rem)', fontWeight: 700, fontFamily: 'var(--font-mono)', color: 'var(--zg-purple-core)' }}>${stats.spotCost}</p></div>
                                  <div><p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: '0.5rem', textTransform: 'uppercase', fontWeight: 600 }}>On-Demand</p><p style={{ margin: 0, fontSize: 'clamp(1rem, 2vw, 1.5rem)', fontWeight: 700, fontFamily: 'var(--font-mono)', color: '#4b5563', textDecoration: 'line-through' }}>${stats.odCost}</p></div>
                                  <div><p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: '0.5rem', textTransform: 'uppercase', fontWeight: 600 }}>Capital Saved</p><p style={{ margin: 0, fontSize: 'clamp(1rem, 2vw, 1.5rem)', fontWeight: 700, fontFamily: 'var(--font-mono)', color: '#10b981' }}>+${stats.savings}</p></div>
                                </div>
                              </div>
                            </motion.div>
                          )}
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '2rem', paddingTop: '1.5rem', borderTop: 'var(--border-subtle)' }}>
                          <p style={{ margin: 0, color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'var(--bg-layer-2)', padding: '0.5rem 1rem', borderRadius: 'var(--radius-sm)' }}>
                            <Server size={14} strokeWidth={1.5} /> EC2_ID: <span style={{ color: 'var(--text-pure)' }}>{env.instance_id || 'PENDING_ALLOCATION'}</span>
                          </p>
      ...
      <AnimatePresence mode="wait">
        {currentWorkspace && activeTab === 'iam' && (
          <motion.div key="iam" variants={containerVariants} initial="hidden" animate="show" exit="hidden" className="glass-panel" style={{ padding: '3rem' }}>
            <motion.h2 variants={itemVariants} style={{ marginTop: 0, color: 'var(--text-pure)', fontSize: 'clamp(1.5rem, 3vw, 2rem)', marginBottom: '1rem', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '1rem' }}>
              <Shield size={32} strokeWidth={1.5} style={{ color: 'var(--zg-purple-core)' }} /> Security & IAM Bindings
            </motion.h2>
            <motion.p variants={itemVariants} style={{ color: 'var(--text-muted)', fontSize: 'clamp(0.8125rem, 1.2vw, 1rem)', marginBottom: '3rem', lineHeight: 1.6, maxWidth: '700px' }}>
            ...
            <motion.form variants={itemVariants} onSubmit={handleSaveIntegration} style={{ display: 'grid', gap: '2rem' }}>
              <div style={{ background: 'var(--bg-layer-2)', border: 'var(--border-subtle)', padding: '1.25rem 1.5rem', borderRadius: 'var(--radius-sm)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ color: 'var(--text-muted)', fontSize: '0.9rem', fontWeight: 600 }}>Targeting Workspace:</span>
                <span style={{ color: 'var(--text-pure)', fontFamily: 'var(--font-mono)', fontSize: '1rem', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <CloudCog size={18} strokeWidth={1.5} style={{ color: 'var(--zg-purple-core)' }} /> {currentWorkspace}
                </span>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                ...
                <input 
                  ...
                  style={{ background: 'var(--bg-layer-2)', border: '1px solid rgba(255,255,255,0.1)', color: 'var(--text-pure)', padding: '1rem 1.25rem', borderRadius: 'var(--radius-sm)', outline: 'none', width: '100%', fontSize: '1rem', fontFamily: 'var(--font-mono)' }}
                />
              </div>
