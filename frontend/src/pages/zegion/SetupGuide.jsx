            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: `1px solid ${isActive ? 'var(--zg-purple-border)' : 'transparent'}`,
                  color: isActive ? 'var(--text-pure)' : isCompleted ? 'var(--text-muted)' : 'var(--text-muted)',
              ...
              >
                <StepIcon size={18} style={{ color: isActive ? 'var(--zg-purple-core)' : isCompleted ? '#10b981' : 'inherit' }} />
            ...
            >
              ...
              {steps[activeStep].code && (
                <div style={{ position: 'relative', marginTop: '2rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.5)', padding: '0.5rem 1rem', borderTopLeftRadius: 'var(--radius-sm)', borderTopRightRadius: 'var(--radius-sm)', border: '1px solid var(--border-subtle)', borderBottom: 'none' }}>
                    <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)', fontFamily: 'monospace' }}>zegion.yml</span>
                    <button 
                      onClick={() => handleCopy(steps[activeStep].code)}
                      style={{ color: copied ? '#10b981' : 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '0.25rem', fontSize: '0.75rem', fontWeight: 600, transition: 'color var(--transition-fast)' }}
                    >
                  ...
                  <pre style={{ margin: 0, backgroundColor: 'var(--bg-void)', padding: '1.5rem', borderRadius: '0 0 var(--radius-sm) var(--radius-sm)', border: '1px solid var(--border-subtle)', overflowX: 'auto' }}>
                    <code style={{ fontFamily: 'monospace', fontSize: '0.9rem', color: 'var(--text-pure)', whiteSpace: 'pre-wrap' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '3rem', paddingTop: '2rem', borderTop: 'var(--border-subtle)' }}>
                <button
                  ...
                  style={{ color: 'var(--text-muted)', fontSize: '0.9rem', fontWeight: 600, opacity: activeStep === 0 ? 0.3 : 1, cursor: activeStep === 0 ? 'not-allowed' : 'pointer', transition: 'color var(--transition-fast)' }}
                ...
                {activeStep < steps.length - 1 ? (
                  <button
                    ...
                    style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', backgroundColor: 'var(--text-pure)', color: 'var(--bg-void)', padding: '0.75rem 1.5rem', borderRadius: 'var(--radius-sm)', fontSize: '0.9rem', fontWeight: 700, transition: 'opacity var(--transition-fast)' }}
                  ...
                  <Link
                    to="/zegion/dashboard"
                    style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', backgroundColor: 'var(--zg-purple-core)', color: 'var(--text-pure)', padding: '0.75rem 1.5rem', borderRadius: 'var(--radius-sm)', fontSize: '0.9rem', fontWeight: 700, transition: 'opacity var(--transition-fast)' }}
                    onMouseEnter={(e) => e.currentTarget.style.opacity = '0.9'}
                    onMouseLeave={(e) => e.currentTarget.style.opacity = '1'}
                  >
